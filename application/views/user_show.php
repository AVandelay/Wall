...
<?php 			if($user_level == ADMIN || $user_id == $user->id)
				{	?>
								<td><a href="/users/edit/<?= $user->id?>" class="btn btn-mini">Edit profile</a></td>
<?php 			}	
				else
				{	?>								
								<td></td>
...
<?php			
				...
				{	?>
						<div class="well">
							<div class="pull-left">
								<h5><a href=""><?= $message["first_name"] ." ". $message["last_name"] ?></a> wrote:</h5>
							</div>
<?php 				if($user_level == ADMIN || $user_id == $message['user_id'])
					{	?>
							<form action="/messages/delete_message/<?= $message['message_id'] ?>" class="pull-right form-horizontal delete_message">
								<input type="submit" value="Delete" class="btn">
							</form>
<?php 				}	?>
							<div class="clearfix"></div>
							<p><?= $message["message"] ?></p>
							<h6 class="muted"><?= timespan(strtotime($message["created_at"]), time()) ." ago"?></h6>
							<div class="replies">
<?php				foreach($replies[$message['message_id']] as $reply)
					{	?>
								<div class="message_reply">
									<div class="pull-left">
										<h5><a href=""><?= $reply["first_name"] ." ". $reply["last_name"] ?></a> replied:</h5>
										<p><?= $reply["message"] ?></p>
										<h6 class="muted"><?= timespan(strtotime($reply["created_at"]),  time()) ." ago"?></h6>
									</div>	
<?php 					if($user_level == ADMIN || $user_id == $reply['user_id'])
						{	?>									
									<form action="/messages/delete_message/<?= $reply['message_id'] ?>" class="pull-right form-horizontal delete_message">
										<input type="submit" value="Delete" class="btn">
									</form>
<?php 					}	?>									
									<div class="clearfix"></div>
...
<?php 				}	?>
							...
							<form action="/messages/reply" class="form-horizontal send_reply" method="post">
								<div class="control-group">
									<input type="hidden" name="recipient_user_id" value="<?= $message['user_id'] ?>">
